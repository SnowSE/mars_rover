...
public class MovementTests
{
	...
	{
		await initializeGame();
		if (player1.StartingX is 0 or 4) //on a side
		{
	...
	{
		factory = IntegrationTestHelper.CreateFactory();
		multiGameHoster = factory.Services.GetRequiredService<MultiGameHoster>();
	...
	private async Task moveAsync(string desiredOrientation, bool shouldSucceed = true)
	{
		...
		{
			PerseveranceMoveResponse turnResult;
			do
			{
